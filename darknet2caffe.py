...
caffe_root='/home/ubuntu223/hezhj/caffe-ssd/'
#os.chdir(caffe_root)
import sys
...
def darknet2caffe(cfgfile, weightfile, protofile, caffemodel):
    ...
    for block in blocks:
        ...
        if block['type'] == 'net':
            ...
            if block.has_key('name'):
                reshape_name = '%s-reshape' % block['name']
                scale_name = '%s-scale' % block['name']
            else:
                reshape_name='layer%d-reshape' % layer_id
                scale_name='layer%d-scale' % layer_id
            layer_id = layer_id+1
        else:
